...
const MyRecipes = ({ searchTerm }) => {  
  ...
  useEffect(() => {
    if (searchTerm) {
      ...
      setFilteredRecipes(recipes); // If no search term, display all recipes
    }
  }, [searchTerm, recipes]);  // Re-run the filter when searchTerm or recipes change
  ...
  const handleDelete = (id) => {
    ...
    axios.delete(`http://localhost:5001/recipes/${id}`)
      .then(() => {
        ...
        setFilteredRecipes(filteredRecipes.filter(recipe => recipe.id !== id)); // Also remove from filteredRecipes
      })
      .catch(error => {
